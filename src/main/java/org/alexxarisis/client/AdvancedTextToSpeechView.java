package org.alexxarisis.client;
...
import org.alexxarisis.commands.CommandsFactory;
import org.alexxarisis.commands.ReplayManager;
import org.alexxarisis.model.Document;
import org.alexxarisis.util.ResourcesUtil;
...
public class AdvancedTextToSpeechView implements ActionListener, ComponentListener {
	 ...
	 * (line wrap capability). Currently only Excel files do not need line wrap.
	 * 
	 * @param contents String the text we want to set.
	 * @param lineWrap Boolean, true if we want it to have line wrap.
	 */
	public void setTextAndLineWrap(String contents, boolean lineWrap) {
		// has line wrap depending on the boolean given
	...
	/**
	 * Disables all the buttons associated with open file commands.
	...
	private JToolBar setupToolbarArea() {
		...
		int height = 32;
		int width = 32;
		JButton openFile 				= new JButton(ResourcesUtil.getIcon("open-folder.png", width, height));
		JButton saveFile 				= new JButton(ResourcesUtil.getIcon("floppy-disk.png", width, height));
		JButton closeFile 				= new JButton(ResourcesUtil.getIcon("close-document.png", width, height));
		JToggleButton editFileToolbar 	= new JToggleButton(ResourcesUtil.getIcon("edit-file.png", width, height));
		JToggleButton selectTextToolbar = new JToggleButton(ResourcesUtil.getIcon("select-no.png", width, height));
		JButton play 					= new JButton(ResourcesUtil.getIcon("play.png", width, height));
		JToggleButton record 			= new JToggleButton(ResourcesUtil.getIcon("record.png", width, height));
		JButton replay 					= new JButton(ResourcesUtil.getIcon("replay.png", width, height));

		// Tooltips
		openFile.setToolTipText("Open file");
		saveFile.setToolTipText("Save file");
		closeFile.setToolTipText("Close currently open file");
		editFileToolbar.setToolTipText("Enable text editing");
		selectTextToolbar.setToolTipText("Enable text selection");
		play.setToolTipText("Play");
		record.setToolTipText("Record plays");
		replay.setToolTipText("Replay recorded plays");

		// selected/not selected icons for the JToggleButtons in the ToolBar
		editFileToolbar.setSelectedIcon(ResourcesUtil.getIcon("stop-edit-file.png", width, height));
		editFileToolbar.setDisabledSelectedIcon(ResourcesUtil.getIcon("edit-file.png", width, height));
		selectTextToolbar.setSelectedIcon(ResourcesUtil.getIcon("select-yes.png", width, height));
		selectTextToolbar.setDisabledSelectedIcon(ResourcesUtil.getIcon("select-no.png", width, height));
		record.setSelectedIcon(ResourcesUtil.getIcon("stop.png", width, height));
		record.setDisabledSelectedIcon(ResourcesUtil.getIcon("record.png", width, height));

		// Remove focus from all
		openFile.setFocusable(false);
		saveFile.setFocusable(false);
		closeFile.setFocusable(false);
		editFileToolbar.setFocusable(false);
		selectTextToolbar.setFocusable(false);
	...
	/**
	 * Creates a JButton with a pre-selected image-icon to be used as the default
	...
	private JMenu setupEdit() {
		...
		// we enable stopEditFile when editFile is pressed
	...
	private JMenu setupView() {
		...
		fontSizeLabel 	 = new JLabel("            " + font.getSize() + " ");
